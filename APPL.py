...
def generate_skyplot(sat_azimuths, sat_elevations, sat_labels=None):
    """生成极坐标卫星星空图，使用 layout.polar 避免参数错误"""
    ...
    for i, (az, el, label) in enumerate(zip(sat_azimuths, sat_elevations, sat_labels)):
        r = 90 - el
        theta = (90 - az) % 360   # 转换为极坐标角度（北为0度，顺时针）
        fig.add_trace(go.Scatterpolar(
            r=[r], theta=[theta],
            mode='markers+text',
    ...
    # 使用 layout.polar 设置极坐标属性（避免 update_polar 的兼容性问题）
    fig.update_layout(
        polar=dict(
            radialaxis=dict(range=[0, 95], showticklabels=False, showgrid=False),
            angularaxis=dict(
                tickmode='array',
